...
class AccessGroupAPITest extends CLPTestCase {
	...
	public function test_can_get_list_of_groups() {
		$groups = $this->client->accessGroups()->getGroups();
	...
	public function test_can_get_single_group() {
		...
		$knownGroup = $this->client->accessGroups()->getGroups()->items()->first();
		$group = $this->client->accessGroups()->getGroup($knownGroup->getID());
	...
	public function test_can_create_update_and_delete_a_group() {
		...
		$createdGroup = $this->client->accessGroups()->createGroup($newGroup);
		...
		$updatedGroup = $this->client->accessGroups()->updateGroup($createdGroup->getID(), $newGroup);
		...
		$refreshedGroup = $this->client->accessGroups()->getGroup($updatedGroup->getID());
		...
		$this->client->accessGroups()->deleteGroup($refreshedGroup->getID());

		$this->expectException(EndpointNotFound::class);

		$remainingGroup = $this->client->accessGroups()->getGroup($updatedGroup->getID());
	...
	public function test_can_get_add_and_remove_time_schedules() {
		...
		$createdSchedule = $this->client->accessGroups()->addTimeSchedule($createdGroup->getID(), $newSchedule);
		...
		$foundSchedules = $this->client->accessGroups()->getTimeSchedules($createdGroup->getID());
		...
		$updatedSchedule = $this->client->accessGroups()->updateTimeSchedule($createdGroup->getID(), $foundSchedule->getID(), $newSchedule);
		...
		$this->client->accessGroups()->removeTimeSchedule($createdGroup->getID(), $updatedSchedule->getID());

		$refreshedSchedules = $this->client->accessGroups()->getTimeSchedules($createdGroup->getID());
		...
		$this->client->accessGroups()->deleteGroup($createdGroup->getID());

	}

	public function test_can_get_add_and_remove_locks() {

		$group = $this->client->accessGroups()->createGroup(new NewAccessGroup());
		$this->assertInstanceOf('Clay\CLP\Structs\AccessGroup', $group);
		...
		$this->client->accessGroups()->updateGroupLocks($group->getID(), [$lock->getID()]);

		$existingLocks = $this->client->accessGroups()->getGroupLocks($group->getID());
		...
		$remainingLocks = $this->client->accessGroups()->getGroupLocks($group->getID());
	...
	public function test_can_get_add_and_remove_accessors() {
		...
		$this->client->accessGroups()->updateGroupAccessors($group->getID(), [$accessor->getID()]);

		$existingAccessors = $this->client->accessGroups()->getGroupAccessors($group->getID());
		...
		$remainingAccessors = $this->client->accessGroups()->getGroupAccessors($group->getID());
